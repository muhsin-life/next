...
const typographyVariants = cva("", {
  variants: {
    variant: {
      ...
      ghost: "text-[#39f] dark:text-white",
      lifeText: "text-life dark:text-slate-50",
      danger: "text-red-500 ",
      paragraph:"text-slate-500 dark:text-slate-300 ",
      default: "",
    },
    whitespace: {
    ...
    size: {
      ...
      xl: "sm:text-2xl text-lg",
      sm: "sm:text-sm text-xs",
      lg: "md:text-lg sm:text-base text-sm",
...
const Typography: React.FC<TypographyProps> = ({
  ...
  return (
    ...
    >
      {children}
      {" "} 
      {requiredField ?<span className={typographyVariants({variant:"danger", size:size})}>*</span> : null}
    </Element>
  );
};
