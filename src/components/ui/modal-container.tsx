...
const ModalContainer = React.forwardRef<HTMLDivElement, ModalProps>(({ showModal, setCloseModal, children, className, size, fullModal }) => {
    ...
    return (
        width > 575 || fullModal?
            ...
            :
            <BottomSheet  snapPoints={({ minHeight, maxHeight }) => [minHeight, maxHeight / 0.1]} className="z-[100]" open={showModal} onDismiss={() => setCloseModal(false)}>
                <div className="bg-white px-3 pb-2 z-[100]">
                    {children}
                </div>
            </BottomSheet>
    )
